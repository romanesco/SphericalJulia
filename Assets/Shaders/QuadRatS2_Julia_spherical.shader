Shader "Custom/Spherical/QuadraticRational-S_2_Julia"
{
    Properties
    {
        _X("Real(c)", float) = -0.5
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float2 f(float2 z, float2 d)
            {
                z = cpx_mult(z,z-float2(1,0));
                return cpx_div(d,z);
            }

            float2 df(float2 z, float2 d)
            {
                float2 z1 = z - float2(1,0);
                float2 num = -cpx_mult(d,2*z1);
                z = cpx_mult(z, z1);
